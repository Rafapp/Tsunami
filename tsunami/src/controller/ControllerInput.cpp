...
StateOfController Controller::poll() {
	...
	state.left_stick_y =
	    processStick(-gp.axes[GLFW_GAMEPAD_AXIS_LEFT_Y]);        // flip: up = positive
	...
	// Triggers
	state.left_trigger =
	    normalizeTrigger(gp.axes[GLFW_GAMEPAD_AXIS_LEFT_TRIGGER], trigger_threshold);
	state.right_trigger =
	    normalizeTrigger(gp.axes[GLFW_GAMEPAD_AXIS_RIGHT_TRIGGER], trigger_threshold);
	...
	state.touchpad_button = gp.buttons[GLFW_GAMEPAD_BUTTON_GUIDE] ==
	                        GLFW_PRESS;        // Is actually the playstation button, apparently
	                                           // GLFW doesn't have the touchpad mapped.
