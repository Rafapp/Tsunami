...
static VkDeviceAddress get_bda(VkDevice dev, VkBuffer buf) {
	...
	return vkGetBufferDeviceAddress(dev, &i);
		...
		static void transition_layout(VkCommandBuffer cmd, VkImage img, VkImageLayout old,
		                              VkImageLayout n, VkAccessFlags src, VkAccessFlags dst,
		...
		static void end_one_time_cmd(VkDevice dev, VkCommandPool pool, VkQueue q,
		                             VkCommandBuffer cmd) {
		...
		// ============================================================
		static void upload_rgba32f_2d(VmaAllocator alloc, VkDevice dev, VkCommandPool pool,
		                              VkQueue q, const float* rgba, uint32_t w, uint32_t h,
		                              LutTexture2D& out) {
			VkDeviceSize  sz = (VkDeviceSize) w * h * 4 * sizeof(float);
			VmaAllocation sa;
			VkBuffer      stg =
			    create_and_upload_buffer(alloc, sz, rgba, VK_BUFFER_USAGE_TRANSFER_SRC_BIT, sa);
			...
			r.imageExtent      = {w, h, 1};
			vkCmdCopyBufferToImage(cmd, stg, out.image, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, 1,
			                       &r);
			...
			out.view   = create_image_view(dev, out.image, VK_FORMAT_R32G32B32A32_SFLOAT,
			                               VK_IMAGE_VIEW_TYPE_2D);
			out.width  = w;
			out.height = h;
			vmaDestroyBuffer(alloc, stg, sa);
		}
		static void upload_rgba32f_3d(VmaAllocator alloc, VkDevice dev, VkCommandPool pool,
		                              VkQueue q, const float* rgba, uint32_t w, uint32_t h,
		                              uint32_t d, LutTexture3D& out) {
		...
		static void     handle_resize(uint32_t & frame_number, uint32_t fb_w, uint32_t fb_h) {
            ...
            vmaDestroyImage(allocator, render_target_ctx.accum_image,
			                    render_target_ctx.accum_image_alloc);

            // -- Rebuild swapchain --
            vkb::Swapchain old_swapchain = swapchain_ctx.swapchain;
            auto           swap_ret      = vkb::SwapchainBuilder{vulkan_ctx.log_device}
                                .set_desired_format(
            ...
            // Reset sync semaphores if the swapchain image count changed
            {
                ...
                if (n != (uint32_t) sync_ctx.image_available.size()) {
                    ...
                    for (uint32_t i = 0; i < n; ++i) {
                        vkCreateSemaphore(vulkan_ctx.device, &si, nullptr,
						                      &sync_ctx.image_available[i]);
                        vkCreateSemaphore(vulkan_ctx.device, &si, nullptr,
						                      &sync_ctx.render_finished[i]);
                    }
                }
            }
		...
		static void upload_openpbr_luts(VmaAllocator alloc, VkDevice dev, VkCommandPool pool,
		                                VkQueue q) {
			// Requires:
			...
			// ---------------------------------------------------------------------
			{
				...
				upload_rgba32f_3d(
				    alloc, dev, pool, q, data.data(), OpenPBR_EnergyTableSize,
				    OpenPBR_EnergyTableSize, OpenPBR_EnergyTableSize,
				    render_target_ctx
				        .lut_textures_3d[OpenPBR_LutId_IdealDielectricEnergyComplement]);
			}

			{
				...
				upload_rgba32f_3d(
				    ...
				    OpenPBR_EnergyTableSize, OpenPBR_EnergyTableSize,
				    render_target_ctx
				        .lut_textures_3d[OpenPBR_LutId_OpaqueDielectricEnergyComplement]);
			}
			...
			{
				auto data =
				    uint_lut_to_rgba32f(OpenPBR_IdealDielectricAverageEnergyComplement_Array,
				                        OpenPBR_EnergyTableSize * OpenPBR_EnergyTableSize);
...

			{
				...
				upload_rgba32f_2d(
				    ...
				    OpenPBR_EnergyTableSize,
				    render_target_ctx
				        .lut_textures_2d[OpenPBR_LutId_IdealDielectricReflectionRatio]);
			}

			{
				auto data =
				    uint_lut_to_rgba32f(OpenPBR_OpaqueDielectricAverageEnergyComplement_Array,
				                        OpenPBR_EnergyTableSize * OpenPBR_EnergyTableSize);
			...
			// ---------------------------------------------------------------------
			{
				...
				upload_rgba32f_2d(
				    ...
				    render_target_ctx
				        .lut_textures_2d[OpenPBR_LutId_IdealMetalAverageEnergyComplement]);
			}
		...
		// ============================================================
		static void upload_material_textures(
		    VmaAllocator alloc, VkDevice dev, VkCommandPool pool, VkQueue q,
			...
			for (const auto& tex : textures) {
				...
				const VkFormat fmt =
				    tex->is_srgb ? VK_FORMAT_R8G8B8A8_SRGB : VK_FORMAT_R8G8B8A8_UNORM;
				VkImageCreateInfo ii{};
				ii.sType         = VK_STRUCTURE_TYPE_IMAGE_CREATE_INFO;
				ii.imageType     = VK_IMAGE_TYPE_2D;
				ii.format        = fmt;
		...
		static BLAS build_blas(VmaAllocator alloc, VkDevice dev, VkCommandPool pool, VkQueue q,
		                       ...
		                       uint32_t ic) {
			VmaAllocation va, ia;
			VkBuffer      vb = create_and_upload_buffer(
                alloc, (VkDeviceSize) vstride * vc, verts,
                AS_INPUT_BUFFER_USAGE | VK_BUFFER_USAGE_VERTEX_BUFFER_BIT, va);
			VkBuffer ib = create_and_upload_buffer(
			    alloc, (VkDeviceSize) sizeof(uint32_t) * ic, idxs,
			...
			vkGetAccelerationStructureBuildSizesKHR(
			    dev, VK_ACCELERATION_STRUCTURE_BUILD_TYPE_DEVICE_KHR, &build, &tc, &si);
			BLAS blas;
			blas.buffer = create_gpu_buffer(alloc, si.accelerationStructureSize, AS_BUFFER_USAGE,
			                                blas.buffer_alloc);
			VmaAllocation sca;
		...
		static void build_tlas(VmaAllocator alloc, VkDevice dev, VkCommandPool pool, VkQueue q,
			...
			VmaAllocation ia;
			VkBuffer      ib = create_and_upload_buffer(
                alloc, sizeof(VkAccelerationStructureInstanceKHR) * insts.size(), insts.data(),
                AS_INPUT_BUFFER_USAGE, ia);
			...
			si.sType = VK_STRUCTURE_TYPE_ACCELERATION_STRUCTURE_BUILD_SIZES_INFO_KHR;
			vkGetAccelerationStructureBuildSizesKHR(
			    dev, VK_ACCELERATION_STRUCTURE_BUILD_TYPE_DEVICE_KHR, &build, &ic, &si);

			VmaAllocation sca;
			as_ctx.tlas_buffer = create_gpu_buffer(alloc, si.accelerationStructureSize,
		...
		App::App() {
			...
			// ========================================
			{
				...
				auto make_dummy = [&](VkImageType itype, VkExtent3D e, VkImage& img,
				                      VmaAllocation& a) {
					VkImageCreateInfo ii{};
					ii.sType         = VK_STRUCTURE_TYPE_IMAGE_CREATE_INFO;
					ii.imageType     = itype;
			...
			// =============================================
			{
				...
				if (vkCreateCommandPool(vulkan_ctx.device, &pi, nullptr,
			...
			// Transition dummy images → SHADER_READ_ONLY once
			{
				VkCommandBuffer cmd =
				    begin_one_time_cmd(vulkan_ctx.device, command_ctx.command_pool);
				for (VkImage img :
				     {render_target_ctx.dummy_image_2d, render_target_ctx.dummy_image_3d})
					transition_layout(cmd, img, VK_IMAGE_LAYOUT_UNDEFINED,
			...
			// =================================
			{
				...
				scene_ctx.camera_buffer = create_and_upload_buffer(allocator, sizeof(GPUCamera),
				                                                   &gc, SB, scene_ctx.camera_alloc);
				...
				scene_ctx.material_buffer =
				    create_and_upload_buffer(allocator, sizeof(GPUMaterial) * gms.size(),
			...
			for (int mi = 0; mi < (int) m_scene->m_meshes.size(); ++mi) {
				...
				all_verts.insert(all_verts.end(), mesh->gpuVertices.begin(),
				                 mesh->gpuVertices.end());
				...
				std::cout << "[INFO] BLAS " << mi << ": " << mesh->gpuIndices.size() / 3
				          << " tris\n";
			}
			...
			if (!gpu_meshes.empty()) {
				...
				scene_ctx.index_buffer =
				    create_and_upload_buffer(allocator, sizeof(uint32_t) * all_idxs.size(),
			...
			//  11  = material_textures[256]  SAMPLED_IMAGE × MAX_MATERIAL_TEXTURES
			{
				...
				if (as_ctx.tlas != VK_NULL_HANDLE)
					bindings.push_back(
					    make_binding(5, VK_DESCRIPTOR_TYPE_ACCELERATION_STRUCTURE_KHR));
				...
				if (vkCreateDescriptorSetLayout(vulkan_ctx.device, &dsl, nullptr,
				                                &render_target_ctx.descriptor_set_layout) !=
				    VK_SUCCESS)
			...
			}
			{
				...
				if (vkAllocateDescriptorSets(vulkan_ctx.device, &ai,
			...
			// Write descriptors
			{
				...
				for (uint32_t i = 0; i < NUM_LUTS; ++i) {
					lut2[i].imageView =
					    (render_target_ctx.lut_textures_2d[i].view != VK_NULL_HANDLE) ?
				...
				for (uint32_t i = 0; i < NUM_LUTS; ++i) {
					lut3[i].imageView =
					    (render_target_ctx.lut_textures_3d[i].view != VK_NULL_HANDLE) ?
				...
				vkUpdateDescriptorSets(vulkan_ctx.device, (uint32_t) writes.size(), writes.data(),
				                       0, nullptr);
			...
			// ============================================
			{
				auto spirv = compile_slang_shader(std::string(SHADERS_DIR) + "/pathtracer.slang",
				                                  "main", {VENDORS_DIR});
			...
			// ==========================
			{
				...
				sync_ctx.render_finished.resize(n);
				for (uint32_t i = 0; i < n; ++i) {
					if (vkCreateSemaphore(vulkan_ctx.device, &sei, nullptr,
					                      &sync_ctx.image_available[i]) != VK_SUCCESS)
						throw std::runtime_error("failed to create semaphore");
					if (vkCreateSemaphore(vulkan_ctx.device, &sei, nullptr,
					                      &sync_ctx.render_finished[i]) != VK_SUCCESS)
						throw std::runtime_error("failed to create semaphore");
				}
	...
	void App::MainLoop() {
		...
		while (!m_window->shouldClose()) {
			...
			if (controls_changed) {
				if (m_audio_controller != nullptr) {
					audio_level =
					    m_audio_controller->update(overlay_ctx.controls.audio, audio_input);
					overlay_ctx.diagnostics.audio = m_audio_controller->diagnostics();
				}
			...
			vkCmdBlitImage(cmd, render_target_ctx.storage_image,
