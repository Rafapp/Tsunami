...
static std::vector<uint32_t> compile_slang_shader(const std::string& path, const std::string& entry_point) {
...
App::App() {
	...
	if (!family_ret) throw std::runtime_error("failed to get graphics queue family");
	...
	if (vmaCreateImage(allocator, &image_info, &alloc_info, &render_target_ctx.storage_image, &render_target_ctx.storage_image_alloc, nullptr) != VK_SUCCESS) {
        throw std::runtime_error("failed to create storage image");
    }
    std::cout << "[INFO] Created storage image (resolution: " << image_info.extent.width << "x" << image_info.extent.height << ")\n";
	...
	if (vkCreateImageView(vulkan_ctx.device, &view_info, nullptr, &render_target_ctx.storage_image_view) != VK_SUCCESS) {
	...
	if (vkCreateDescriptorSetLayout(vulkan_ctx.device, &dsl_info, nullptr, &render_target_ctx.descriptor_set_layout) != VK_SUCCESS) {
	...
	if (vkCreateDescriptorPool(vulkan_ctx.device, &pool_info, nullptr, &render_target_ctx.descriptor_pool) != VK_SUCCESS) {
	...
	if (vkAllocateDescriptorSets(vulkan_ctx.device, &descriptor_alloc_info, &render_target_ctx.descriptor_set) != VK_SUCCESS) {
    ...
    std::cout << "[INFO] Compiled compute shader to SPIR-V (" << (spirv.size() * sizeof(uint32_t)) << " bytes)\n";
	...
	if (vkCreatePipelineLayout(vulkan_ctx.device, &pipeline_layout_info, nullptr, &compute_ctx.pipeline_layout) != VK_SUCCESS) {
	...
	if (vkCreateComputePipelines(vulkan_ctx.device, VK_NULL_HANDLE, 1, &pipeline_info, nullptr, &compute_ctx.pipeline) != VK_SUCCESS) {
	...
	if (vkCreateCommandPool(vulkan_ctx.device, &cmd_pool_info, nullptr, &command_ctx.command_pool) != VK_SUCCESS) {
	...
	if (vkAllocateCommandBuffers(vulkan_ctx.device, &cmd_alloc_info, &command_ctx.command_buffer) != VK_SUCCESS) {
...
vkWaitForFences(vulkan_ctx.device, 1, &sync_ctx.in_flight, VK_TRUE, UINT64_MAX);
        ...
        vkCmdBindPipeline(command_ctx.command_buffer, VK_PIPELINE_BIND_POINT_COMPUTE, compute_ctx.pipeline);
        vkCmdBindDescriptorSets(
            command_ctx.command_buffer,
            VK_PIPELINE_BIND_POINT_COMPUTE,
            compute_ctx.pipeline_layout,
            0, 1, &render_target_ctx.descriptor_set,
            0, nullptr
        );
        ...
        blit.dstOffsets[1]  = { (int32_t)swapchain_ctx.swapchain.extent.width, (int32_t)swapchain_ctx.swapchain.extent.height, 1 };
