    ...
    Scene& dst)
{
    ...
    if (ai_mat->GetTexture(type, 0, &tex_path) != aiReturn_SUCCESS)
        return 0xFFFFFFFFu;
    auto tex = Texture::load_from_assimp(ai_scene, scene_dir, tex_path, srgb);
    if (!tex || !tex->valid())
        return 0xFFFFFFFFu;
...
static void load_all_material_textures(
    const aiScene* ai_scene,
    const std::string& scene_dir,
    Scene& dst)
{
    ...
    auto load_slot = [&](aiMaterial* mat, aiTextureType type, bool srgb) {
        ...
        for (unsigned int i = 0; i < count; ++i) {
            aiString tex_path;
            if (mat->GetTexture(type, i, &tex_path) != aiReturn_SUCCESS) continue;
            auto tex = Texture::load_from_assimp(ai_scene, scene_dir, tex_path, srgb);
            if (tex && tex->valid()) {
                dst.load_texture_if_needed(tex);
                std::cout << "[Scene] Loaded texture: " << tex->source_path
...
static void wire_texture_indices_node_recursive(
    const aiScene* ai_scene,
    const aiNode* node,
    const std::string& scene_dir,
    ...
    int& mesh_idx)
{
    for (unsigned int i = 0; i < node->mNumMeshes; ++i) {
        ...
        if (mat) {
            ...
            uint32_t alb = get_tex_index(ai_scene, scene_dir, ai_mat, aiTextureType_BASE_COLOR, true, dst);
            if (alb == 0xFFFFFFFFu)
                alb = get_tex_index(ai_scene, scene_dir, ai_mat, aiTextureType_DIFFUSE, true, dst);
            mat->m_gpu.albedo_tex_index = alb;

            uint32_t nrm = get_tex_index(ai_scene, scene_dir, ai_mat, aiTextureType_NORMALS, false, dst);
            if (nrm == 0xFFFFFFFFu)
                nrm = get_tex_index(ai_scene, scene_dir, ai_mat, aiTextureType_HEIGHT, false, dst);
            mat->m_gpu.normal_tex_index = nrm;

            uint32_t rgh = get_tex_index(ai_scene, scene_dir, ai_mat, aiTextureType_DIFFUSE_ROUGHNESS, false, dst);
            if (rgh == 0xFFFFFFFFu)
                rgh = get_tex_index(ai_scene, scene_dir, ai_mat, aiTextureType_UNKNOWN, false, dst);
            mat->m_gpu.roughness_tex_index = rgh;

            mat->m_gpu.emissive_tex_index =
                get_tex_index(ai_scene, scene_dir, ai_mat, aiTextureType_EMISSIVE, true, dst);
        }
    ...
    for (unsigned int c = 0; c < node->mNumChildren; ++c) {
        wire_texture_indices_node_recursive(
            ai_scene, node->mChildren[c], scene_dir, meshes, dst, mesh_idx);
    }
}

static void wire_texture_indices(
    const aiScene* ai_scene,
    const std::string& scene_dir,
    std::vector<std::unique_ptr<Mesh>>& meshes,
    Scene& dst)
{
    int mesh_idx = 0;
    wire_texture_indices_node_recursive(
        ai_scene, ai_scene->mRootNode, scene_dir, meshes, dst, mesh_idx);
}
...
void Scene::load_gltf(const std::string& path) {
    ...
    std::cout << "[Scene] Meshes after geometry pass: " << m_meshes.size() << "\n";
    ...
    if (!ai_scene || !ai_scene->mRootNode) {
        std::cerr << "[Scene::load_gltf] Assimp texture pass failed: "
                  << importer.GetErrorString() << "\n";
        return;
    }

    const std::string scene_dir = fs::path(path).parent_path().string();

    // Load all referenced textures into m_textures (deduplication via m_texture_lookup).
    load_all_material_textures(ai_scene, scene_dir, *this);

    // Wire per-mesh texture indices now that every texture has a stable index.
    wire_texture_indices(ai_scene, scene_dir, m_meshes, *this);

    std::cout << "[Scene] Meshes: " << m_meshes.size()
