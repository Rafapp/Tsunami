...
static uint32_t get_tex_index(const aiScene* ai_scene, const std::string& scene_dir,
...
static void load_all_material_textures(const aiScene* ai_scene, const std::string& scene_dir,
                                       Scene& dst) {
	if (!ai_scene)
		return;
...
static void wire_texture_indices_node_recursive(const aiScene* ai_scene, const aiNode* node,
	...
	for (unsigned int c = 0; c < node->mNumChildren; ++c) {
		wire_texture_indices_node_recursive(ai_scene, node->mChildren[c], scene_dir, meshes, dst,
		                                    mesh_idx);
	}
}

static void wire_texture_indices(const aiScene* ai_scene, const std::string& scene_dir,
                                 std::vector<std::unique_ptr<Mesh>>& meshes, Scene& dst) {
	int mesh_idx = 0;
	wire_texture_indices_node_recursive(ai_scene, ai_scene->mRootNode, scene_dir, meshes, dst,
	                                    mesh_idx);
}
...
void Scene::load_gltf(const std::string& path) {
	...
	if (!ai_scene || !ai_scene->mRootNode) {
		std::cerr << "[Scene::load_gltf] Assimp texture pass failed: " << importer.GetErrorString()
		          << "\n";
		return;
	}
