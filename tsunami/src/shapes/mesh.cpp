...
static constexpr uint32_t ASSIMP_OBJ_FLAGS =
...
static void collect_node_transforms(const aiNode* node, const aiMatrix4x4& parent_transform,
                                    std::vector<aiMatrix4x4>& out_transforms,
                                    uint32_t mesh_count) {
...
Mesh::Mesh(std::vector<GPUVertex> verts, std::vector<uint32_t> indices,
           Transform transform, std::shared_ptr<Material> material)
...
std::vector<std::unique_ptr<Mesh>> Mesh::load_gltf(const std::string& path) {
    ...
    for (unsigned int m = 0; m < scene->mNumMeshes; ++m) {
        ...
        for (unsigned int i = 0; i < ai_mesh->mNumVertices; ++i) {
            GPUVertex gv{};
            gv.position = {ai_mesh->mVertices[i].x,
                           ai_mesh->mVertices[i].y,
                           ai_mesh->mVertices[i].z};
            gv.normal   = {ai_mesh->mNormals[i].x,
